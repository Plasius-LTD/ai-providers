...
- Invocations contain a canonical `ModelRequestSpec`, one to 20 immutable candidates, public canonical `mcp://models/...` PNG evidence, an absolute deadline, and an optional `AbortSignal` at execution time. Preview resources must be scoped to the same canonical catalog asset/version identity or staged resolution candidate; catalog identity is validated by the shared asset-contract factories. Untrusted arrays must be dense concrete data arrays and untrusted objects must contain only expected enumerable data properties; caller-overridden array iteration is never used for normalization.
